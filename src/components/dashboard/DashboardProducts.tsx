...
const DashboardProducts = () => {
  ...
  const openEdit = (p: Product) => { setForm({ name: p.name, description: p.description, price: p.price.toString(), image: p.image, category: p.category, inStock: p.inStock }); setEditId(p.id); setShowForm(true); };
  ...
  const confirmDelete = (id: string) => { deleteProduct(id); setDeleteConfirm(null); };
    ...
    <div className="space-y-6">
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-background border rounded-xl p-6 w-full max-w-md shadow-xl animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Price (ZMW)</Label><Input type="number" step="0.01" required value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} /></div>
                <div>
                  <Label>Category</Label>
                  <select required value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  {form.category === '__new' && (
                    <Input className="mt-2" placeholder="Enter new category" onChange={(e) => setForm({ ...form, category: e.target.value })} />
                  )}
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2 mt-1 mb-2">
                  <button type="button" onClick={() => setImageMode('url')} className={`text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors ${imageMode === 'url' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
                    <LinkIcon className="h-3 w-3" /> URL
                  </button>
                  <button type="button" onClick={() => setImageMode('file')} className={`text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors ${imageMode === 'file' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
                    <Upload className="h-3 w-3" /> From Device
                  </button>
                </div>
                {imageMode === 'url' ? (
                  <Input required={!form.image} value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} placeholder="https://..." />
                ) : (
                  <>
                    <input
                      ref={fileInputRef}
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.inStock} onChange={(e) => setForm({ ...form, inStock: e.target.checked })} className="rounded" /> In Stock</label>
      ...
      {Array.from(new Set(products.map(p => p.category))).sort().map(category => (
        <div key={category} className="space-y-3">
          <h3 className="font-display text-sm font-semibold text-muted-foreground uppercase tracking-wider border-b pb-2">{category} ({products.filter(p => p.category === category).length})</h3>
